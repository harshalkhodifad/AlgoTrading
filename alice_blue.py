...
class CryptoJsAES:
    ...
    def encrypt(message, passphrase):
        ...
        return base64.b64encode(
            b"Salted__" + salt + aes.encryptor().update(CryptoJsAES.__pad(message)) + aes.encryptor().finalize())
...
class AliceBlue:
    ...
    __urls = {"webLogin": f"{host}/customer/webLogin",
              ...
              "ws_InvalidateSession": f"{host}/api/ws/invalidateSocketSess",
    ...
    @staticmethod
    def login_and_get_sessionID(username=None, password=None, twoFA=None, app_id=None, api_secret=None):
    ...
    def __extract_tick_data(self, data):
        ...
        if ("ft" in data):  # Feed Time
            self.__tick_data[data["instrument"].symbol]["exchange_time_stamp"] = datetime.datetime.fromtimestamp(
                int(data.pop("ft")))
    ...
    def __extract_depth_data(self, data):
        ...
        if ("ltt" in data):  # Last Traded Time
            self.__depth_data[data["instrument"].symbol]["last_traded_time"] = datetime.datetime.strptime(
                data.pop("ltt"), "%H:%M:%S").time()
    ...
    def start_websocket(self, subscribe_callback=None,
        ...
        # self.__api_call_helper('ws_InvalidateSession', Requests.POST, data)
        ...
        # Send initial data
        data = {"susertoken": hashlib.sha256(
            hashlib.sha256(self.__session_id.encode('utf-8')).hexdigest().encode('utf-8')).hexdigest(),
                "t": "c",
    ...
    def cancel_order(self, order_id, leg_order_id=None):
        ...
        if (leg_order_id == None):
            ...
            return self.__api_call_helper('exitBracketOrder', Requests.POST,
                                          {"nestOrderNumber": order_id, "symbolOrderId": leg_order_id,
                                           "status": "open"})
            ...
            if (exch != "contract_date"):
                for scrip in body[exch]:
                    ...
                    if "lot_size" in scrip:
                        try:
                            lot_size = int(scrip["lot_size"])
                        except:
                            lot_size = None

                    if "option_type" in scrip:
                        option_type = scrip["option_type"]
                    else:
                        option_type = None

                    if "instrument_type" in scrip:
                        instrument_type = scrip["instrument_type"]
                    else:
                        instrument_type = None

                    if "strike_price" in scrip:
                        strike_price = float(scrip["strike_price"])
                    else:
                        strike_price = None
    ...
    def __api_call(self, url, http_method, data):
        ...
        headers = {"X-SAS-Version": "2.0",
                   "Content-Type": "application/json",
                   "Authorization": f"Bearer {self.__username.upper()} {self.__session_id}"}
        r = None
